...
### Contents 1 Version information 1
...
5 Matplotlib 3 5.1 plotting of a simple graph . . . . . . . . . . . . . . . . . . . . 3 5.2 Plotting from an Org table . . . . . . . . . . . . . . . . . . . 4

6 Pandas 5 6.1 printing a data frame as a table (and noweb block inclusion) . 5 6.1.1 data frame printing using Ipython.display . . . . . . . 7 6.1.2 an older and simpler dataFrame printing alternative: . 8 6.2 plotting a data frame (and placing a code reference) . . . . . 9 6.3 time series resampling . . . . . . . . . . . . . . . . . . . . . . 10 7 Sympy 11
...
1 (princ ( concat 2(format "Emacs version: %s\n" 3(emacs-version)) 1
...
 [http://orgmode.org/worg/org-contrib/babel/languages/ob-doc-python](http://orgmode.org/worg/org-contrib/babel/languages/ob-doc-python). html
...
3 4 5 6 7 81 2 3 6 9 12 15 18 21 24 27
...
time = epoch import datetime strtime = str (time) datetimestamp = datetime .datetime.utcfromtimestamp(int(strtime[:10])) print datetimestamp .strftime('[%Y-%m-%d %a %H:%M:%S]' )

[2010-01-05 Tue 07:11:05]

In the table we need to refer to the named source block by using the a short lisp form involvingorg-sbe function is to be interpreted as a number, the reference uses a single dollar sign, e.g. $1 (as here). If it should be interpreted as a string, one puts an additional dollar sign in front, e.g. $$1.

epoch day 1262675465119 123456 99998754

### 5 Matplotlib 5.1 plotting of a simple graph
...
. If the table value that is referred to in the

2010-01-05 Tue 07:11 1970-01-02 Fri 10:17 1973-03-03 Sat 09:25
...
fname='python-matplot-fig2.png' ar = np .array(data).transpose() fig=plt.figure(figsize=(4,2)) plt.plot(ar[0],ar[1]) plt.title('Square function' ) fig.tight_layout() plt.savefig(fname) # return filename to org-modereturn fname

x y 1 1 2 4 3 9 4 16 5 25 6 36 7 49 4
...
hlines_tmp.append(1) else: for hl in hlines: if hl < 0 : hlines_tmp.append(len(lines.split('\n')) + hl) else: hlines_tmp.append(hl)

for i,l in enumerate (lines.split('\n')): if i in hlines_tmp: print "|-----" print l

In the following source block, I demonstrate how to use the of including a named block within another, by referring to our DataFrame printing block bydframeToOrg
...
for i,l in enumerate (lines.split('\n')): if i in hlines_tmp: print "|-----" print l df = pd .DataFrame({'A' : [ 'one', , 'two' , 'three' ] * 3 , 'B' : [ 'A', 'B' , 'C' ] * 4 , 'C' : [ 'foo', , 'foo' , 'bar' , 'bar' , 'bar' ] * 2 , 'D' : np .random.randn(12), 'E' : np .random.randn(12)})
...
As an alternative, the display function from Ipython is also able to align a frame. I only managed to getdiplay_pretty output is lacking table separators. So, it only displays nicely in an example environment.
...
working up to now, and its

).

---

The displayand displayfunctions produce no output.latexhtml

import pandas as pd import numpy as np from IPython.display import display_pretty

df = pd .DataFrame({'A' : [ 'one', , 'two' , 'three' ] * 3 , 'B' : [ 'A', 'B' , 'C' ] * 4 , 'C' : [ 'foo', , 'foo' , 'bar' , 'bar' , 'bar' ] * 2 , 'D' : np .random.randn(12), 'E' : np .random.randn(12)})
...
df = pd .DataFrame({'A' : [ 'one', , 'two' , 'three' ] * 3 , 'B' : [ 'A', 'B' , 'C' ] * 4 , 'C' : [ 'foo', , 'foo' , 'bar' , 'bar' , 'bar' ] * 2 ,
...
newdf = tdf .resample('1D', loffset ='6h',how='min').rename(columns={'value': '1D_resample' }) newdf['diff'] = newdf .diff() 10
...
value 1D_resample diff 2013-07-01 06:00:00 0 0 NaN 2013-07-02 06:00:00 20 20 20 2013-07-03 06:00:00 40 40 20 2013-07-05 06:00:00 80 80 NaN 2013-07-06 06:00:00 100 100 20 2013-07-07 06:00:00 120 120 20 2013-07-08 06:00:00 NaN 150 30 2013-07-09 06:00:00 160 160 10 2013-07-10 06:00:00 180 180 20

### 7 Sympy

I dene a post-wrapping function for putting the results into the desired equation environment.

cat <<EOF \begin{equation} $outp \end{equation} EOF

AThe correct preview of the resulting LT X fragment I only get with theE output drawerresults options. I tested rendering with the Aoption, but the resulting LT X block is not rendered by theE command ( C-c C-x C-l ).
...
Z 1 dx x AThe above L T X equation is also rendered nicely in the HTML export.E 11
...
try: print strg except: print "Expected error:" , sys .exc_info()[0]
...
print 'default encoding is now %s ' % sys .getdefaultencoding() try: print strg except: 13
